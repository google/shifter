...
func (server *Server) setupRouter() {
	...
	v1 := router.Group("/api/v1")
	{
		...
		o := v1.Group("/openshift")
		{
			...
			rt := o.Group("/route")
			{
				rt.POST("/:projectName", server.SOSGetRoutesByProject)
				rt.POST("/:projectName/:routeName", server.SOSGetRoute)
			}
			cm := o.Group("/configmap")
			{
				cm.POST("/:projectName", server.SOSGetConfigMapsByProject)
				cm.POST("/:projectName/:configmapName", server.SOSGetConfigMap)
			}
			bd := o.Group("/build")
			{
				bd.POST("/:projectName", server.SOSGetBuildsByProject)
				bd.POST("/:projectName/:buildName", server.SOSGetBuild)
			}
		}
