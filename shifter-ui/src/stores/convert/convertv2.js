...
export const useConvertObjects = defineStore(
  "shifter-api-v1-convertv2-objects",
  {
    state: () => {
      return {
        cluster: {},
          ...
          if (state.conversionItems.length >= 1) {
            var idx = state.conversionItems.findIndex((object) => {
              ...
              if (item.UID !== undefined) {
                return object.resource.UID === item.UID;
              }
            });
      ...
      async add(resource) {
        try {
          const newConversionItem = {
            namespace: storeOSProjects.getByName(resource.Namespace),
            resource: resource,
          };
        ...
        if (this.conversionItems.length >= 1) {
          var idx = this.conversionItems.findIndex((object) => {
            ...
            if (item.metadata === undefined) {
              return object.resource.UID === item.resource.UID;
            }
          });
          if (idx >= 0) {
